...
namespace M2MqttUnity
{
    ...
    public class M2MqttUnityClient : MonoBehaviour
    {
        ...
        [HideInInspector] public string brokerAddress = "localhost";
        [Tooltip("Port where the broker accepts connections")]
        [HideInInspector] public int brokerPort = 1883;
        [Tooltip("Use encrypted connection")]
        [HideInInspector] public bool isEncrypted = false;
        [Header("Connection parameters")]
        [Tooltip("Connection to the broker is delayed by the given milliseconds")]
        [HideInInspector] public int connectionDelay = 500;
        [Tooltip("Connection timeout in milliseconds")]
        [HideInInspector] public int timeoutOnConnection = MqttSettings.MQTT_CONNECT_TIMEOUT;
        ...
        [HideInInspector] public string mqttUserName = null;
        [Tooltip("Password for the MQTT broker. Keep blank if no password is required.")]
        [HideInInspector] public string mqttPassword = null;
        ...
        public bool MQTTClientConnected => mqttClientConnected;
        ...
        /// </summary>
        /// <returns>The execution is done asynchronously.</returns>
        private async Task DoConnectAsync()
        {
            ...
            try
            {
                await Task.Run(() => client.Connect(clientId, mqttUserName, mqttPassword));
            }
            catch (Exception e)
            {
                client = null;
